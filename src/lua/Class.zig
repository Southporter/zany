...
fn connectSignalFromStack(state: *lua.Lua, class: *Class, name: []const u8, ud: i32) void {
    ...
    for (class.signals.items) |*signal| {
        if (signal.id == id) {
            signal.emit(state, 1);
...
pub fn emitSignal(class: *Class, state: *lua.Lua, name: [:0]const u8, nargs: i32) void {
